	...
	<div class="page-container">
		<div class="page-header">
			...
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow(),modalList.addRow = true, genPrivateKey(),genPrivateIv()" :loading="loading" v-if="$p.permissionChecker('packageManagementAdd')">{{$t('menu.app_tools_package_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('packageManagementDelete')">{{$t('menu.app_tools_package_delete')}}</el-button>
			</div>
		</div>
			...
			<el-card shadow="never">
				<div class="page-filter">
					...
					<div class="p-3 d-flex flex-wrap">
						...
						<el-select class="custom-input fixed-width-200 m-2" size="large" v-model="searchData.package" :placeholder="$t('mix.table_please_select')+$t('mix.table_package')" clearable @change="initial()">
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('packageManagementEdit')) ? false : true"></el-switch>
							</template>

							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('packageManagementEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('packageManagementDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
			...
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				...
				<template v-if="userDetails.login !== ''">
						...
						<el-col :sm="12" class="mb-3">
							...
							<el-select class="custom-input mt-1 w-100" v-model="postForm.package" :placeholder="$t('mix.table_package')" size="large">
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_event_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					...
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_package')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.package" :placeholder="$t('mix.table_package')" disabled/>
					</el-col>
...
export default {
	...
	methods:{
		getInitial(){
			...
			var result = this.$m.postMethod('packages/management/package',this.postData)
			result.then((value)=>{
				var data = value.data
				if(value.valid){
			...
			var result = this.$m.postMethod('packages/management/package/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			if(this.$p.permissionChecker('packageManagementAdd') && this.loading == false){
				this.loading = true
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('packages/management/package/add',this.postData)
				result.then((value)=>{
					var data = value.data
			...
			if(this.$p.permissionChecker('packageManagementAdd') && this.loading == false){
                ...
                this.postData.data = JSON.stringify(this.postForm)
                var result = this.$m.postMethod('packages/management/package/DBadd',this.postData)
			...
			if((this.$p.permissionChecker('packageManagementEdit')) && this.loading == false){
				this.loading = true

				this.submitForm.id = id
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('packages/management/package/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if((this.$p.permissionChecker('packageManagementEdit')) && this.loading == false){
                ...
                var result = this.$m.postMethod('packages/management/package/DBedit',this.postData)
                result.then((value) => {
                    var data = value.data
			...
			if(this.$p.permissionChecker('packageManagementDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("packages/management/package/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data
							if (value.valid) {
								this.$message({
			...
			if(this.$p.permissionChecker('packageManagementEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('packages/management/package/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('packageManagementAdd') && this.loading == false){
				this.loading = true
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('packages/management/package/DBsearchUser',this.postData)
				result.then((value) => {
