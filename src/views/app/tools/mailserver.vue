<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_mailserver')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="loading" v-if="$p.permissionChecker('toolMailServerAdd')">{{$t('menu.app_tools_mailserver_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolMailServerDelete')">{{$t('menu.app_tools_mailserver_delete')}}</el-button>
			</div>
		</div>
			...
			<el-card shadow="never">
				<div class="page-filter">
					...
					<div class="p-3 d-flex flex-wrap">
						...
						<el-select class="custom-input fixed-width-200 m-2" v-model="searchData.country_id" :placeholder="$t('mix.table_country')" clearable @change="initial()">
							<el-option v-for="item in countryList" :key="item.id" :label="item.name" :value="item.id">{{item.name}}</el-option>
						</el-select>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolMailServerEdit')) ? false : true"></el-switch>
							</template>

							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolMailServerEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolMailServerDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
        ...
        <el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_mailserver_add')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>	
				<el-row :gutter="20">
                    ...
                    <el-col :sm="12" class="mb-3">
                        ...
                        <el-select class="custom-input w-100 mt-1" v-model="postForm.country_id" :placeholder="$t('mix.table_country')">
                            <el-option :label="$t('msg.msg_select')" :value="0">{{$t('msg.msg_select')}}</el-option>
                            <el-option v-for="country in countryList" :label="country.name" :value="country.id">{{country.name}}</el-option>
                        </el-select>
                    </el-col>
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_mailserver_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :span="24" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	methods:{
		getInitial(){
            ...
            var result = this.$m.postMethod('app/tools/mailserver',this.postData)
            result.then((value) => {
                var data = value.data
			...
			var result = this.$m.postMethod('app/tools/mailserver/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			if(this.$p.permissionChecker('toolMailServerAdd') && this.loading == false){
				this.loading = true
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/mailserver/add',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolMailServerAdd') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/mailserver/DBadd',this.postData)
			...
			if(this.$p.permissionChecker('toolMailServerEdit') && this.loading == false){
				this.loading = true
				this.submitForm.id = id
				
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/mailserver/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolMailServerEdit') && this.loading == false){
				...
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/mailserver/DBedit',this.postData)
			...
			if(this.$p.permissionChecker('toolMailServerEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/mailserver/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolMailServerDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/mailserver/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
