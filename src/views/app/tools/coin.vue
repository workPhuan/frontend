<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_coin')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="loading" v-if="$p.permissionChecker('toolCoinAdd')">{{$t('menu.app_tools_coin_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolCoinDelete')">{{$t('menu.app_tools_coin_delete')}}</el-button>
			</div>
		</div>
				...
				<el-table :data="tableData" v-loading="loading" class="custom-table mt-3" ref="tableTest" :show-header="true" @selection-change="handleSelectionChange">
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'address'" #default="scope">
								<p v-for="(item,index) in scope.row.address">{{index}}: {{item}}</p>
							</template>
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolCoinEdit')) ? false : true"></el-switch>
							</template>
							...
							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolCoinEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolCoinDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
		...
		<el-dialog v-model="modalList.addRow" v-if="modalList.addRow" :title="$t('menu.app_tools_coin_add')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					...
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_price')}}(USDT)</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.rate" :step="1" :min="0" :precision="8"/>
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold">{{$t('mix.table_contract_address')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.contract_address" :placeholder="$t('mix.table_contract_address')" />
					</el-col>

					<el-col :sm="12" class="mb-3" v-for="item in networkList">
						<label class="text-theme font-8 fw-bold">{{item.name}}{{ $t('mix.table_deposit_address') }}</label>
						<el-input class="custom-input mt-1" v-model="postForm.network_list[item.id]" :placeholder="item.name+$t('mix.table_deposit_address')" />
					</el-col>
					...
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_min_deposit')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.min_deposit" :step="1" :min="0" :precision="8"/>
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_min_withdraw')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.min_withdraw" :step="1" :min="0" :precision="8"/>
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_max_withdraw')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.max_withdraw" :step="1" :min="0" :precision="8"/>
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_min_transfer')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.min_transfer" :step="1" :min="0" :precision="8"/>
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_max_transfer')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.max_transfer" :step="1" :min="0" :precision="8"/>
					</el-col>

					<el-col :sm="12" class="mb-3" v-if="0">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_network_fee')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.network_fee" :step="1" :min="0" :precision="2"/>
					</el-col>
		...
		<el-dialog v-model="modalList.editRow" v-if="modalList.editRow" :title="$t('menu.app_tools_coin_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	data(){
		return {
			...
			ajaxTitles:[{
                ...
                prop:"address",
                label:this.$t('mix.table_deposit_address'),
                width:'250',
				align:'left'
			},{
                prop:"rate",
                label:this.$t('mix.table_price')+"(USDT)",
                width:'100',
				align:'left'
			},{
                prop:"sort",
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/coin/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			this.postForm.code = ''
			this.postForm.contract_address = ''
			this.postForm.deposit_address = ''
			this.postForm.network_list = {}
			this.postForm.rate = 0
			this.postForm.rate_rmb = 0
			this.postForm.min_deposit = 0
			this.postForm.min_withdraw = 0
			this.postForm.max_withdraw = 0
			this.postForm.min_transfer = 0
			this.postForm.max_transfer = 0
			this.postForm.network_fee = 0
			...
			this.loading = true
			this.postData.data = JSON.stringify(this.searchData)
			var result = this.$m.postMethod('app/tools/coin/add',this.postData)
			result.then((value) => {
				var data = value.data
			...
			if(this.$p.permissionChecker('toolCoinAdd') && this.loading == false){
				this.loading = true
				this.preloader(true)
				...
				var result = this.$m.postMethod('app/tools/coin/DBadd',formData)
			...
			if(this.$p.permissionChecker('toolCoinEdit') && this.loading == false){
				this.loading = true
				this.submitForm.id = id
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/coin/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolCoinEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/coin/DBedit',formData)
			...
			if(this.$p.permissionChecker('toolCoinEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/coin/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
		...
		},depositRow(currentData){
			if(this.$p.permissionChecker('toolCoinEdit') && this.loading == false){
				this.loading = true

                var form = {}
                form.id = currentData.id
                form.is_deposit = currentData.is_deposit
				...
				var result = this.$m.postMethod('app/tools/coin/DBdeposit',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
		...
		},withdrawRow(currentData){
			if(this.$p.permissionChecker('toolCoinEdit') && this.loading == false){
				this.loading = true

                var form = {}
                form.id = currentData.id
                form.is_withdraw = currentData.is_withdraw
				this.postData.data = JSON.stringify(form)
				...
				var result = this.$m.postMethod('app/tools/coin/DBwithdraw',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolCoinDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						var result = this.$m.postMethod("app/tools/coin/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
