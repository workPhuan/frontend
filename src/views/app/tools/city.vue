<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_city')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="buttonLoading" v-if="$p.permissionChecker('toolCityAdd')">{{$t('menu.app_tools_city_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="buttonLoading" v-if="$p.permissionChecker('toolCityDelete')">{{$t('menu.app_tools_city_delete')}}</el-button>
			</div>
		</div>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type" :fixed="(title.prop == 'action') ? 'right' : false">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolCityStatus')) ? false : true"></el-switch>
							</template>
							...
							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolCityEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolCityDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
        ...
        <el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_city_add')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>	
				<el-row :gutter="20">
                    ...
                    <el-col :sm="12" class="mb-3">
						...
						<el-select class="custom-input mt-1 w-100" v-model="postForm.country_id" filterable @change="changeCountry()">
							<el-option v-for="(item,key) in countryList" :key="key" :label="item.name" :value="item.id">{{item.name}}</el-option>
						</el-select>
					</el-col>

                    <el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_state')}}</label>
						<el-select class="custom-input mt-1 w-100" v-model="postForm.state_id" filterable>
							<el-option v-for="(item,key) in stateList" :key="key" :label="item.name" :value="item.id">{{item.name}}</el-option>
						</el-select>
					</el-col>
			...
			<template #footer>
				<div class="d-flex justify-content-center align-item-center">
					...
					<el-button class="custom-button danger font-8 pt-3 pb-3" @click="modalList.addRow = false,clearPostForm()" :loading="buttonLoading">{{$t('button.close')}}</el-button>
				</div>
			</template>
        </el-dialog>

		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_city_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :span="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
			...
			<template #footer>
				<div class="d-flex justify-content-center align-item-center">
					...
					<el-button class="custom-button danger font-8 pt-3 pb-3" @click="modalList.editRow = false,clearPostForm()" :loading="buttonLoading">{{$t('button.close')}}</el-button>
				</div>
			</template>
        </el-dialog>
	</div>
...
export default {
	...
	methods:{
        getInitial(){
            ...
            var result = this.$m.postMethod('app/tools/city', this.postData)
            result.then((value) => {
                var data = value.data
		...
		initial(){
			...
			var result = this.$m.postMethod('app/tools/city/ajaxTable', this.postData)
			result.then((value) => {
				var data = value.data
            ...
            if(this.$p.permissionChecker('toolCityAdd')){
                this.buttonLoading = true
                
                this.postData.data = JSON.stringify(this.submitForm)
                var result = this.$m.postMethod('app/tools/city/add',this.postData)
                result.then((value) => {
                    var data = value.data
			...
			if(this.$p.permissionChecker('toolCityAdd')){
				...
				var result = this.$m.postMethod('app/tools/city/DBadd', this.postData)
			...
			if(this.$p.permissionChecker('toolCityEdit')){
				this.buttonLoading = true
				this.submitForm.id = id
				
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/city/edit', this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolCityEdit')){
				...
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/city/DBedit', this.postData)
			...
			if(this.$p.permissionChecker('toolCityEdit')){
				...
				var result = this.$m.postMethod('app/tools/city/DBstatus', formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolCityDelete')){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$prompt(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postData.data = JSON.stringify(this.postForm)
						var result = this.$m.postMethod("app/tools/city/DBdelete", this.postData)
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
            ...
            this.countryList.forEach((item)=>{
                if(item.id == this.postForm.country_id){
                    this.stateList = item.state_list
                }
            })
        },initialState(){
            this.stateList = []
