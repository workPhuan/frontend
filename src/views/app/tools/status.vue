<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_status')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="loading" v-if="$p.permissionChecker('toolStatusAdd')">{{$t('menu.app_tools_status_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolStatusDelete')">{{$t('menu.app_tools_status_delete')}}</el-button>
			</div>
		</div>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolStatusEdit')) ? false : true"></el-switch>
							</template>

							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolStatusEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolStatusDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
        ...
        <el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_status_add')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>	
				<el-row :gutter="20">
					...
					<el-col :sm="12" class="mb-3">
						...
						<div class="status-switch-prepend-box">
							<el-switch class="status-switch-prepend" v-model="postForm.switch" size="small" />
							<el-input v-if="postForm.switch" class="custom-input" v-model="postForm.category" :placeholder="$t('mix.table_category')"/>
							<el-select v-if="!postForm.switch" class="custom-input w-100" v-model="postForm.category">
								<el-option v-for="item in categoryList" :key="item" :label="item" :value="item">{{item}}</el-option>
							</el-select>
						</div>
					</el-col>
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_status_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :span="24" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/status/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			this.postForm.switch = false
			if(done != undefined){
				done()
			}
		},getAddRow(){
			if(this.$p.permissionChecker('toolStatusAdd') && this.loading == false){
				this.loading = true
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/status/add',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolStatusAdd') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/status/DBadd',this.postData)
			...
			if(this.$p.permissionChecker('toolStatusEdit') && this.loading == false){
				this.loading = true
				this.submitForm.id = id
				
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/status/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolStatusEdit') && this.loading == false){
				...
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/status/DBedit',this.postData)
			...
			if(this.$p.permissionChecker('toolStatusEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/status/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolStatusDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/status/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
