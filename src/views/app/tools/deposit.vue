<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_deposit')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="modalList.addRow = true" :loading="loading" v-if="$p.permissionChecker('toolDepositAdd')">{{$t('menu.app_tools_deposit_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolDepositDelete')">{{$t('menu.app_tools_deposit_delete')}}</el-button>
			</div>
		</div>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'deposit_address'" #default="scope">
								<p>{{$t('mix.table_erc20_address')}}: {{scope.row.erc20_address}}</p>
								<p>{{$t('mix.table_trc20_address')}}: {{scope.row.trc20_address}}</p>
							</template>
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolDepositEdit')) ? false : true"></el-switch>
							</template>
							...
							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolDepositEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolDepositDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
		...
		<el-dialog v-model="modalList.addRow" v-if="modalList.addRow" :title="$t('menu.app_tools_deposit_add')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					...
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold">{{$t('mix.table_type')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.type" :placeholder="$t('mix.table_type')" />
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_min_deposit_amount')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.min_amount" :step="1" :min="0" :precision="0"/>
					</el-col>

                    <el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_max_deposit_amount')}}</label>
						<el-input-number class="custom-input mt-1 w-100" v-model="postForm.max_amount" :step="1" :min="0" :precision="0"/>
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_callback_url')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.callback_url" :placeholder="$t('mix.table_callback_url')" />
					</el-col>
		...
		<el-dialog v-model="modalList.editRow" v-if="modalList.editRow" :title="$t('menu.app_tools_deposit_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	data(){
		return {
			...
			ajaxTitles:[{
                ...
                prop:"min_amount",
                label:this.$t('mix.table_min_deposit_amount'),
                width:'100',
				align:'left'
			},{
                prop:"max_amount",
                label:this.$t('mix.table_max_deposit_amount'),
                width:'100',
				align:'left'
			},{
                prop:"sort",
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/deposit/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			this.postForm.code = ''
			this.postForm.type = ''
			this.postForm.min_amount = 0
			this.postForm.max_amount = 0
			...
			if(this.$p.permissionChecker('toolDepositAdd') && this.loading == false){
				this.loading = true
				this.preloader(true)
				...
				var result = this.$m.postMethod('app/tools/deposit/DBadd',formData)
			...
			if(this.$p.permissionChecker('toolDepositEdit') && this.loading == false){
				this.loading = true
				this.submitForm.id = id
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/deposit/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolDepositEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/deposit/DBedit',formData)
			...
			if(this.$p.permissionChecker('toolDepositEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/deposit/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolDepositDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						var result = this.$m.postMethod("app/tools/deposit/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
