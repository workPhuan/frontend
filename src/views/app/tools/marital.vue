<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_marital')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="loading" v-if="$p.permissionChecker('toolAttributeAdd')">{{$t('menu.app_tools_marital_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolAttributeDelete')">{{$t('menu.app_tools_marital_delete')}}</el-button>
			</div>
		</div>
		...
		<el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_marital_add')" :before-close="clearPostForm">
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_marital_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :span="24" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/marital/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			if(this.$p.permissionChecker('toolAttributeAdd') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/marital/add',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolAttributeAdd') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/marital/DBadd',this.postData)
			...
			if(this.$p.permissionChecker('toolAttributeEdit') && this.loading == false){
				...
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/marital/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolAttributeEdit') && this.loading == false){
				...
				this.preloader(true)
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/marital/DBedit',this.postData)
			...
			if(this.$p.permissionChecker('toolAttributeEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/marital/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolAttributeDelete') && this.loading == false){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/marital/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
