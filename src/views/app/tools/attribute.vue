			...
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					...
					<el-col :sm="12" class="mb-3">
						...
						<el-select class="custom-input mt-1 w-100" v-model="postForm.category" :placeholder="$t('mix.table_category')">
							<el-option :label="$t('mix.table_bonus')" value="bonus">{{$t('mix.table_bonus')}}</el-option>
							<el-option :label="$t('mix.table_feature')" value="feature">{{$t('mix.table_feature')}}</el-option>
						</el-select>
					</el-col>
					
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_filter')}}</label>
						<el-select class="custom-input mt-1 w-100" v-model="postForm.filter" :placeholder="$t('mix.table_filter')">
...
export default {
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/attribute/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			if(this.$p.permissionChecker('toolAttributeAdd') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/attribute/add',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolAttributeAdd') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/attribute/DBadd',this.postData)
			...
			if(this.$p.permissionChecker('toolAttributeEdit') && this.loading == false){
				...
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/attribute/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolAttributeEdit') && this.loading == false){
				...
				this.preloader(true)
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/attribute/DBedit',this.postData)
			...
			if(this.$p.permissionChecker('toolAttributeEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/attribute/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolAttributeDelete') && this.loading == false){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/attribute/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
