<template>_mailchannel
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_mailchannel')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="modalList.addRow = true" :loading="loading" v-if="$p.permissionChecker('toolMailChannelAdd')">{{$t('menu.app_tools_mailchannel_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolMailChannelDelete')">{{$t('menu.app_tools_mailchannel_delete')}}</el-button>
			</div>
		</div>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolMailChannelEdit')) ? false : true"></el-switch>
							</template>

							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolMailChannelEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolMailChannelDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
        ...
        <el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_mailchannel_add')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>	
				<el-row :gutter="20">
                    ...
                    <el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_from_email')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.from_email" :placeholder="$t('mix.table_from_email')" />
					</el-col>

                    <el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_from_name')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.from_name" :placeholder="$t('mix.table_from_name')" />
					</el-col>

                    <el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_api_key')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.api_key" :placeholder="$t('mix.table_api_key')" />
					</el-col>

                    <el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_api_secret')}}</label>
						<el-input class="custom-input mt-1" v-model="postForm.api_secret" :placeholder="$t('mix.table_api_secret')" />
					</el-col>
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_mailchannel_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :span="24" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	data(){
		return {
			...
			ajaxTitles:[{
                ...
                prop:"from_email",
                label:this.$t('mix.table_from_email'),
                width:'150',
            },{
                prop:"from_name",
                label:this.$t('mix.table_from_name'),
                width:'150',
			},{
                prop:"sort",
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/mailchannel/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			this.postForm.code = ''
			this.postForm.from_email = ''
			this.postForm.from_name = ''
			this.postForm.api_key = ''
			this.postForm.api_secret = ''
			...
			if(this.$p.permissionChecker('toolMailChannelAdd')){
				this.preloader(true)
				this.loading = true
				
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/mailchannel/DBadd',this.postData)
			...
			if(this.$p.permissionChecker('toolMailChannelEdit')){
				this.loading = true
				this.submitForm.id = id
				
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/mailchannel/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolMailChannelEdit')){
				...
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/mailchannel/DBedit',this.postData)
			...
			if(this.$p.permissionChecker('toolMailChannelEdit')){
				...
				var result = this.$m.postMethod('app/tools/mailchannel/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolMailChannelDelete')){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/mailchannel/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
