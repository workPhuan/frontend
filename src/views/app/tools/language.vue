	...
	<div class="page-container">
		<div class="page-header">
			...
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="modalList.addRow = true" :loading="loading" v-if="$p.permissionChecker('toolRoleAdd')">{{$t('menu.app_tools_role_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolRoleDelete')">{{$t('menu.app_tools_role_delete')}}</el-button>
			</div>
		</div>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolRoleEdit')) ? false : true"></el-switch>
							</template>

							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolRoleEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolRolePermission')" class="custom-button warning m-1" @click="getPermissionRow(scope.row.id)">{{$t('button.permission')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolRoleDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
...
export default {
	...
	methods:{
		initial() {
			...
			var result = this.$m.postMethod("app/tools/role/ajaxTable", this.postData);
			result.then((value) => {
				var data = value.data;
		...
		clearPostForm(done){
			...
			if(this.$p.permissionChecker('toolRoleAdd') && this.loading == false){
				this.preloader(true)
				this.loading = true
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod("app/tools/role/DBadd", this.postData)
			...
			if(this.$p.permissionChecker('toolRoleEdit') && this.loading == false){
				this.loading = true;
				this.submitForm.id = id;
				this.postData.data = JSON.stringify(this.submitForm);
				var result = this.$m.postMethod("app/tools/role/edit", this.postData);
				result.then((value) => {
					var data = value.data;
		...
		editRow() {
			if(this.$p.permissionChecker('toolRoleEdit') && this.loading == false){
				...
				this.preloader(true)
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod("app/tools/role/DBedit", this.postData)
			...
			if(this.$p.permissionChecker('toolRoleEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/role/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolRoleDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/role/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data
							if (value.valid) {
								this.$message({
			...
			if(this.$p.permissionChecker('toolRolePermission') && this.loading == false){
				this.loading = true;
				this.submitForm.id = id
				this.postData.data = JSON.stringify(this.submitForm);
				var result = this.$m.postMethod("app/tools/role/permission", this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolRolePermission') && this.loading == false){
				...
				this.$confirm(this.$t('msg.msg_confirmation'), this.$t('msg.prompt'), {
					...
					var result = this.$m.postMethod("app/tools/role/DBpermission", this.postData)
