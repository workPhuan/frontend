<template>_mailchannel
	<div class="page-container">app_tools_smschannel
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_smschannel')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="modalList.addRow = true" :loading="loading" v-if="$p.permissionChecker('toolSmsChannelAdd')">{{$t('menu.app_tools_smschannel_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolSmsChannelDelete')">{{$t('menu.app_tools_smschannel_delete')}}</el-button>
			</div>
		</div>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolSmsChannelEdit')) ? false : true"></el-switch>
							</template>

							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolSmsChannelEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolSmsChannelDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
        ...
        <el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_smschannel_add')" :before-close="clearPostForm">
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_smschannel_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :span="24" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/smschannel/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			if(this.$p.permissionChecker('toolSmsChannelAdd')){
				this.preloader(true)
				this.loading = true
				
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/smschannel/DBadd',this.postData)
			...
			if(this.$p.permissionChecker('toolSmsChannelEdit')){
				this.loading = true
				this.submitForm.id = id
				
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/smschannel/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolSmsChannelEdit')){
				...
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('app/tools/smschannel/DBedit',this.postData)
			...
			if(this.$p.permissionChecker('toolSmsChannelEdit')){
				...
				var result = this.$m.postMethod('app/tools/smschannel/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolSmsChannelDelete')){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/smschannel/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
