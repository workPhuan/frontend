<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_memberpackage')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="modalList.addRow = true" :loading="loading" v-if="$p.permissionChecker('toolMemberPackageAdd')">{{$t('menu.app_tools_memberpackage_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolMemberPackageDelete')">{{$t('menu.app_tools_memberpackage_delete')}}</el-button>
			</div>
		</div>
			...
			<el-card shadow="never">
				<div class="page-filter">
					...
					<div class="p-3 d-flex flex-wrap">
						...
						<el-input class="custom-input fixed-width-200 m-2" v-model="searchData.type" :placeholder="$t('mix.table_please_enter')+$t('mix.table_type')" @keyup.enter.native="initial()">
							<template #prepend>
								<label>{{$t('mix.table_type')}}</label>
							</template>
						</el-input>

						<el-select v-model="searchData.category" class="custom-input fixed-width-200 m-2" :placeholder="$t('mix.table_category')" clearable @change="initial()" size='large'>
							<el-option :label="$t('mix.table_sticker')" value="Sticker">{{$t('mix.table_sticker')}}</el-option>
							<el-option :label="$t('mix.table_group')" value="Group">{{$t('mix.table_group')}}</el-option>
						</el-select>

						<el-input class="custom-input fixed-width-200 m-2" v-model="searchData.period" :placeholder="$t('mix.table_please_enter')+$t('mix.table_period')" @keyup.enter.native="initial()">
							<template #prepend>
								<label>{{$t('mix.table_period')}}</label>
							</template>
						</el-input>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolMemberPackageEdit')) ? false : true"></el-switch>
							</template>
							
							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolMemberPackageEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolMemberPackageDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
		...
		<el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_memberpackage_add')" :before-close="clearPostForm">
				...
				<el-row :gutter="20">
					...
					<el-col :sm="12" class="mb-3">
						...
						<el-select class="custom-input mt-1 w-100" v-model="postForm.type" :placeholder="$t('mix.table_type')" size="large">
							<el-option :label="$t('menu.app_tools_memberpackage')" value="member-package">{{$t('menu.app_tools_memberpackage')}}</el-option>
						</el-select>
					</el-col>

					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_category')}}</label>
						<el-select class="custom-input mt-1 w-100" v-model="postForm.category" :placeholder="$t('mix.table_category')" size="large">
							<el-option :label="$t('mix.table_sticker')" value="Sticker">{{$t('mix.table_sticker')}}</el-option>
							<el-option :label="$t('mix.table_group')" value="Group">{{$t('mix.table_group')}}</el-option>
						</el-select>
					</el-col>
					<el-col :span="12" class="mb-3">
						...
						<el-input class="custom-input mt-1" v-model="postForm.period" :placeholder="$t('mix.table_period')" />
					</el-col>
					...
					<el-col :span="24" class="mb-3">
						<template v-if="postForm.multiple_language === 0">
							...
							<label class="text-theme font-8 fw-bold mt-2"><span class="text-danger">*</span> {{$t('mix.table_description')}}</label>
							<el-input type="textarea" class="custom-input mt-1" v-model="postForm.single_description" :placeholder="$t('mix.table_description')" />
						</template>
						...
						<template v-else>
							<el-tabs type="border-card" class="mt-3 mb-4">
								<el-tab-pane v-for="item in languageList" :key="item.id" :item="item" :label="item.name">
									...
									<label class="text-theme font-8 fw-bold mt-2"><span class="text-danger">*</span> {{$t('mix.table_description')}}</label>
									<el-input type="textarea" class="custom-input mt-1" v-model="postForm.description[item.id]" :placeholder="$t('mix.table_description')" />
								</el-tab-pane>
							</el-tabs>
						</template>
					</el-col>
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_memberpackage_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :span="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	methods:{
		initial() {
			...
			var result = this.$m.postMethod("app/tools/memberpackage/ajaxTable", this.postData);
			result.then((value) => {
				var data = value.data;
		...
		clearPostForm(done){
			...
			if(this.$p.permissionChecker('toolMemberPackageAdd') && this.loading == false){
				this.loading = true
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/memberpackage/add',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolMemberPackageAdd') && this.loading == false){
				...
				var result = this.$m.postMethod("app/tools/memberpackage/DBadd", this.postData)
			...
			if(this.$p.permissionChecker('toolMemberPackageEdit') && this.loading == false){
				this.loading = true;
				this.submitForm.id = id;
				this.postData.data = JSON.stringify(this.submitForm);
				var result = this.$m.postMethod("app/tools/memberpackage/edit", this.postData);
				result.then((value) => {
					var data = value.data;
		...
		editRow() {
			if(this.$p.permissionChecker('toolMemberPackageEdit') && this.loading == false){
				...
				this.preloader(true)
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod("app/tools/memberpackage/DBedit", this.postData)
			...
			if(this.$p.permissionChecker('toolMemberPackageEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/memberpackage/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolMemberPackageDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("app/tools/memberpackage/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data
							if (value.valid) {
								this.$message({
