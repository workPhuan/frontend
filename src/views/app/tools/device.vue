<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_device')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="modalList.addRow = true" :loading="loading" v-if="$p.permissionChecker('toolDeviceAdd')">{{$t('menu.app_tools_device_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolDeviceDelete')">{{$t('menu.app_tools_device_delete')}}</el-button>
			</div>
		</div>
			...
			<el-card shadow="never">
				<div class="page-filter">
					...
					<div class="p-3 d-flex flex-wrap">
						<el-input class="custom-input fixed-width-200 m-2" v-model="searchData.code" :placeholder="$t('mix.table_code')" clearable @change="initial()">
						</el-input>
				...
				<el-table :data="tableData" v-loading="loading" class="custom-table mt-3" ref="tableTest" :show-header="true" @selection-change="handleSelectionChange">
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-else-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolAccountEdit')) ? false : true"></el-switch>
							</template>
							
							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolDeviceEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolDeviceDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
        ...
        <el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_device_add')" class="dialog-md" :before-close="clearPostForm">
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_device_edit')" class="dialog-md" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_code')}}</label>
...
export default {
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('app/tools/device/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
				if(value.valid){
					this.defaultImage = data.defaultImage
					this.postForm.img_url = data.defaultImage
			...
			if(this.$p.permissionChecker('toolDeviceAdd') && this.loading == false){
				this.loading = true
				this.preloader(true)
				...
				var result = this.$m.postMethod('app/tools/device/DBadd',formData)
			...
			if(this.$p.permissionChecker('toolDeviceEdit') && this.loading == false){
				this.loading = true
				this.submitForm.id = id
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/device/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolDeviceEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/device/DBedit',formData)
			...
			if(this.$p.permissionChecker('toolDeviceEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/device/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolDeviceDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						var result = this.$m.postMethod("app/tools/device/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
