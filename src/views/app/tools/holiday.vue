<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_tools_holiday')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="loading" v-if="$p.permissionChecker('toolHolidayAdd')">{{$t('menu.app_tools_holiday_add')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('toolHolidayDelete')">{{$t('menu.app_tools_holiday_delete')}}</el-button>
			</div>
		</div>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type" @selection-change="handleSelectionChange">
							...
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('toolHolidayEdit')) ? false : true"></el-switch>
							</template>
							...
							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('toolHolidayEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>
								<el-button v-if="$p.permissionChecker('toolHolidayDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
        ...
        <el-dialog v-model="modalList.addRow" :title="$t('menu.app_tools_holiday_add')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>	
				<el-row :gutter="20">
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_date')}}</label>
						<el-date-picker class="custom-input mt-1 w-100" size="large" v-model="postForm.holiday_at" format="YYYY-MM-DD" value-format="YYYY-MM-DD" type="date":placeholder="$t('mix.table_date')" />
					</el-col>
                    ...
                    <el-col :sm="12" class="mb-3">
						...
						<el-select class="custom-input w-100 mt-1" v-model="postForm.country" size="large">
							<el-option v-for="item in countryList" :label="item.name" :value="item.iso_code_2">{{item.name}}</el-option>
						</el-select>
					</el-col>
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.app_tools_holiday_edit')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
				<el-row :gutter="20">
					<el-col :sm="12" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_date')}}</label>
...
export default {
	...
	methods:{
		getInitial(){
            ...
            var result = this.$m.postMethod('app/tools/holiday',this.postData)
            result.then((value) => {
                var data = value.data
			...
			var result = this.$m.postMethod('app/tools/holiday/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
            ...
            if(this.$p.permissionChecker('toolHolidayAdd') && this.loading == false){
                this.loading = true
                
                this.postData.data = JSON.stringify(this.submitForm)
                var result = this.$m.postMethod('app/tools/holiday/add',this.postData)
                result.then((value) => {
                    var data = value.data
			...
			if(this.$p.permissionChecker('toolHolidayAdd') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/holiday/DBadd',formData)
			...
			if(this.$p.permissionChecker('toolHolidayEdit') && this.loading == false){
				this.loading = true
				this.submitForm.id = id
				
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('app/tools/holiday/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('toolHolidayEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/holiday/DBedit',formData)
			...
			if(this.$p.permissionChecker('toolHolidayEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('app/tools/holiday/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('toolHolidayDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						var result = this.$m.postMethod("app/tools/holiday/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data;
							if (value.valid) {
								this.$message({
