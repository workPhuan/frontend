<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.app_media_gallery')}}
			</label>
		</div>
			...
			<el-card shadow="never">
				<template #header>
					<div class="d-flex justify-content-between align-items-center">
						<div class="left-area">
							...
							<el-popover v-if="$p.permissionChecker('mediaGalleryAdd')" placement="bottom" width="200" style="margin-left:10px; margin-right: 10px;" trigger="click">
								<template #reference>
							...
							<el-button v-if="$p.permissionChecker('mediaGalleryDelete')" type="danger" :loading="buttonloading" @click="deleteRow"><i class="fa-light fa-trash"></i></el-button>
						</div>
							
						<div class="right-area">
							<el-popover v-if="$p.permissionChecker('mediaGalleryUpload')" placement="bottom-start" width="380" style="margin-left:10px;" trigger="click">
								<template #reference>
				...
				<div v-if="galleryList.length > 0"> 
					<el-checkbox-group v-model="dataList.urlList">
						<b-row>
							<b-col cols="6" md="4" lg="3" v-for="(item,index) in galleryList" :key="index" class="pb-4">
								<div class="img-box">
									<template v-if="item.type == 'file'" >
										<div class="text-center img-body">
											<el-image :src="item.path" :preview-src-list="imageList" ></el-image>
...
export default{
	...
	data(){
		...
		initial(){
			...
			var result = this.$m.postMethod('app/media/gallery/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
				if(value.valid){
			...
			if(this.$p.permissionChecker('mediaGalleryDelete')){
				this.loading = true;
				this.$prompt(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
					...
					var result = this.$m.postMethod('app/media/gallery/DBdelete',this.postData)
					result.then((value) => {
						var data = value.data
						if(value.valid){
							this.$message({
			...
			if(this.$p.permissionChecker('mediaGalleryAdd')){
				this.loading = true
				this.buttonloading = true
				
				this.postData.data = JSON.stringify(this.createList)
				
				var result = this.$m.postMethod('app/media/gallery/DBadd',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('mediaGalleryUpload')){
				this.postData.data = JSON.stringify(this.createList)
		...
		this.uploadImageURL = config.url+'app/media/gallery/DBupload'
