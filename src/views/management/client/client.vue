<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.management_client_client')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="loading" v-if="$p.permissionChecker('userChatRoleEdit')">{{$t('button.add_client')}}</el-button>
			</div>
		</div>
...
export default{
	...
	methods:{
		getInitial(){
			...
			var result = this.$m.postMethod('management/client/client',this.postData)
			result.then((value) => {
				var data = value.data
			...
			this.postData.data = JSON.stringify(this.searchData)
			var result = this.$m.postMethod('management/client/client/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
