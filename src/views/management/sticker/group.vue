<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.management_sticker_group')}}
			</label>
			<div class="page-toolbar">
				...
				<el-button class="custom-button plain" @click="getAddRow(),modalList.addRow = true" :loading="loading" v-if="$p.permissionChecker('stickerGroupAdd')">{{$t('mix.table_add_group')}}</el-button>
				<el-button class="custom-button plain" @click="deleteRow('multiple')" :loading="loading" v-if="$p.permissionChecker('stickerGroupDelete')">{{$t('mix.table_delete_group')}}</el-button>
			</div>
		</div>
		...
		<div class="page-body p-3">
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
                            ...
                            <template v-if="title.prop == 'is_popup'" #default="scope">
								<el-switch v-model="scope.row.is_popup" active-value="1" inactive-value="0" @change="popupRow(scope.row)" :disabled="($p.permissionChecker('stickerGroupEdit')) ? false : true"></el-switch>
							</template>
							
							<template v-if="title.prop == 'status'" #default="scope">
								<el-switch v-model="scope.row.status" active-value="1" inactive-value="0" @change="statusRow(scope.row)" :disabled="($p.permissionChecker('stickerGroupEdit')) ? false : true"></el-switch>
							</template>

							<template v-else-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('stickerGroupEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.id)">{{$t('button.edit')}}</el-button>

								<!-- <el-button v-if="1" class="custom-button warning m-1" @click="getPermissionRow(scope.row.id)">{{$t('button.permission')}}</el-button> -->
								<el-button v-if="$p.permissionChecker('stickerGroupDelete')" class="custom-button danger m-1" @click="deleteRow(scope.row.id)">{{$t('button.delete')}}</el-button>
							</template>
						</el-table-column>
					</template>
				</el-table>
            ...
            <el-dialog v-model="modalList.addRow" :title="$t('mix.table_add_group')" :before-close="clearPostForm">
                <el-form label-position="top" label-width="auto" @submit.native.prevent>
                    <el-row :gutter="20">
                        ...
                        <el-col :sm="12" class="mb-3">
                            ...
                            <el-input-number class="custom-input mt-1 w-100" v-model="postForm.price" :min="0" :precision="2" :step="0.1"/>
                        </el-col>
		...
		<el-dialog v-model="modalList.editRow" :title="$t('mix.table_edit_group')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>
                <el-row :gutter="20">

                        <el-col :sm="12" class="mb-3">
                            <label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_price')}}</label>
                            <el-input class="custom-input mt-1 w-100" v-model="postForm.price" :min="0" :precision="2" :step="0.1" disabled/>
                        </el-col>
						...
						<el-col :sm="12" class="mb-3">
							<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_language')}}</label>
							<el-select class="custom-input mt-1 w-100" v-model="postForm.multiple_language" size="large">
								<el-option :label="$t('mix.table_single_language')" :value="0">{{$t('mix.table_single_language')}}</el-option>
								<el-option :label="$t('mix.table_multiple_language')" :value="1">{{$t('mix.table_multiple_language')}}</el-option>
							</el-select>
						</el-col>
...
export default {
	...
	methods:{
		initial() {
			...
			var result = this.$m.postMethod("management/sticker/stickergroup/ajaxTable", this.postData);
			result.then((value) => {
				var data = value.data;
			...
			if(this.$p.permissionChecker('stickerGroupAdd') && this.loading == false){
				this.loading = true
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('management/sticker/stickergroup/add',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('stickerGroupAdd') && this.loading == false){
				...
				var result = this.$m.postMethod("management/sticker/stickergroup/DBadd",this.postData)
			...
			if(this.$p.permissionChecker('stickerGroupEdit') && this.loading == false){
				this.loading = true;
				this.submitForm.id = id;
				this.postData.data = JSON.stringify(this.submitForm);
				var result = this.$m.postMethod("management/sticker/stickergroup/edit", this.postData);
				result.then((value) => {
					var data = value.data;
			...
			if(this.$p.permissionChecker('stickerGroupEdit') && this.loading == false){
				...
				this.preloader(true)
                this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod("management/sticker/stickergroup/DBedit", this.postData)
			...
			if(this.$p.permissionChecker('stickerGroupEdit') && this.loading == false){
				...
				var result = this.$m.postMethod('management/sticker/stickergroup/DBstatus',formData)
				result.then((value) => {
					var data = value.data
					if(value.valid){
						this.$message({
			...
			if(this.$p.permissionChecker('stickerListEdit') && this.loading == false){
				this.loading = true
				var result = this.$m.postMethod("management/sticker/stickergroup/DBresetRedis", this.postData);
				result.then((value) => {
					var data = value.data;
			...
			if(this.$p.permissionChecker('stickerListEdit') && this.loading == false){
				this.loading = true
				var result = this.$m.postMethod("management/sticker/stickergroup/DBclearRedis", this.postData);
				result.then((value) => {
					var data = value.data;
			...
			if(this.$p.permissionChecker('stickerGroupDelete') && this.loading == false){
				var valid = true
				if(id == 'multiple'){
				...
				if(valid){
					...
					this.$confirm(this.$t('msg.msg_delete'), this.$t('msg.prompt'), {
						...
						this.postForm.id = id
						this.postData.data = JSON.stringify(this.postForm);
						var result = this.$m.postMethod("management/sticker/stickergroup/DBdelete", this.postData);
						result.then((value) => {
							var data = value.data
							if (value.valid) {
								this.$message({
