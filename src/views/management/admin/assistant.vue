<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.management_admin_assistant')}}
			</label>
			<div class="page-toolbar">
				<el-button class="custom-button plain" @click="getAddRow()" :loading="loading" v-if="$p.permissionChecker('adminAssistantAdd')">{{$t('menu.management_admin_assistant_add')}}</el-button>
			</div>
		</div>
			...
			<el-card shadow="never">
				<div class="page-filter">
					...
					<div class="p-3 d-flex flex-wrap">
                        ...
                        <el-input class="custom-input fixed-width-200 m-2" v-model="searchData.agent" :placeholder="$t('mix.table_please_enter')+$t('mix.table_agent')" @keyup.enter.native="initial()">
							<template #prepend>
								<label>{{$t('mix.table_agent')}}</label>
							</template>
						</el-input>
					...
					<template v-for="title in ajaxTitles" :key="title.prop">
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
							...
							<template v-if="title.prop == 'action'" #default="scope">
								<el-button v-if="$p.permissionChecker('adminAssistantEdit')" class="custom-button primary m-1" @click="getEditRow(scope.row.master_id)">{{$t('menu.management_admin_assistant_edit')}}</el-button>
								<!-- <el-button v-if="$p.permissionChecker('adminAssistantRoom') && scope.row.is_streamer == true" class="custom-button success m-1" @click="storeTempID = scope.row.master_id,$router.push('/management/admin/room')">{{$t('menu.management_admin_assistant_room')}}</el-button> -->
								<el-button v-if="$p.permissionChecker('adminAssistantPassword')" class="custom-button primary m-1" @click="getPasswordRow(scope.row.master_id, scope.row.login)">{{$t('menu.management_admin_assistant_password')}}</el-button>
								<el-button v-if="$p.permissionChecker('adminAssistantSecurity') && securityCheck == 1" class="custom-button primary m-1" @click="getSecurityRow(scope.row.master_id, scope.row.login)">{{$t('menu.management_admin_assistant_security')}}</el-button>
								<el-button v-if="$p.permissionChecker('adminAssistantPermission')" class="custom-button warning m-1" @click="getPermissionRow(scope.row.master_id, scope.row.login)">{{$t('menu.management_admin_assistant_permission')}}</el-button>
								<el-button v-if="$p.permissionChecker('adminAssistantStatus')" class="custom-button m-1 " :class="(scope.row.status == 'normal') ? 'danger' : 'success'" @click="statusRow(scope.row.master_id)">
		...
		<el-dialog v-model="modalList.addRow" :title="$t('menu.management_admin_assistant_add')" :before-close="clearPostForm" class="dialog-md">
			<el-form label-position="top" label-width="auto" @submit.native.prevent class="submit-form">
				<el-row :gutter="20">
					<el-col :sm="24" class="mb-3" v-if="!isAgent">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_agent')}} ({{ $t('mix.table_username') }})</label>
						<el-input class="custom-input mt-1" v-model="postForm.agent" :placeholder="$t('mix.table_agent')" />
					</el-col>
		...
		<el-dialog v-model="modalList.editRow" :title="$t('menu.management_admin_assistant_edit')" :before-close="clearPostForm" class="dialog-md">
			<el-form label-position="top" label-width="auto" @submit.native.prevent class="submit-form">
				<el-row :gutter="20">
					<el-col :sm="24" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_username')}}</label>
					...
					<el-col :sm="24" class="mb-3" v-if="$p.permissionChecker('adminAssistantStatus')">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_status')}}</label>
						<el-select class="custom-input mt-1 w-100" v-model="postForm.status" :placeholder="$t('msg.msg_select')" filterable>
							<el-option :label="$t('mix.table_normal')" :value="'normal'">{{$t('mix.table_normal')}}</el-option>
							<el-option :label="$t('mix.table_suspended')" :value="'suspended'">{{$t('mix.table_suspended')}}</el-option>
		...
		<el-dialog v-model="modalList.passwordRow" :title="$t('menu.management_admin_assistant_password')" :before-close="clearPostForm">
		...
		<el-dialog v-model="modalList.securityRow" :title="$t('menu.management_admin_assistant_security')" :before-close="clearPostForm">
			<el-form label-position="top" label-width="auto" @submit.native.prevent>	
				<el-row :gutter="20">
					<el-col :sm="24" class="mb-3">
						<label class="text-theme font-8 fw-bold"><span class="text-danger">*</span> {{$t('mix.table_username')}}</label>
...
export default{
	...
	methods:{
		getInitial(){
			...
			var result = this.$m.postMethod('management/admin/assistant',this.postData)
			result.then((value) => {
				var data = value.data
			...
			this.postData.data = JSON.stringify(this.searchData)
			var result = this.$m.postMethod('management/admin/assistant/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
			...
			if(this.$p.permissionChecker('adminAssistantAdd') && this.loading == false){
				this.loading = true
				this.postData.data = JSON.stringify(this.postForm)
				var result = this.$m.postMethod('management/admin/assistant/add',this.postData)
				result.then((value)=>{
					var data = value.data
			...
			if(this.$p.permissionChecker('adminAssistantAdd') && this.loading == false){
                ...
                var result = this.$m.postMethod('management/admin/assistant/DBadd',this.postData)
			...
			if(this.$p.permissionChecker('adminAssistantEdit') && this.loading == false){
				this.loading = true

				this.submitForm.master_id = masterID
				this.postData.data = JSON.stringify(this.submitForm)
				var result = this.$m.postMethod('management/admin/assistant/edit',this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('adminAssistantEdit') && this.loading == false){
                ...
                var result = this.$m.postMethod('management/admin/assistant/DBedit',this.postData)
			...
			if(this.$p.permissionChecker('adminAssistantPassword')){
				this.modalList.passwordRow = true
				this.postForm.username = username
				this.postForm.master_id = masterID
			}
		},passwordRow(){
			if(this.$p.permissionChecker('adminAssistantPassword') && this.loading == false){
				this.loading = true
                this.preloader(true)
                
                this.postData.data = JSON.stringify(this.postForm)

                var result = this.$m.postMethod('management/admin/assistant/DBpassword',this.postData)
			...
			if(this.$p.permissionChecker('adminAssistantSecurity')){
				this.modalList.securityRow = true
				this.postForm.username = username
				this.postForm.master_id = masterID
			}
		},securityRow(){
			if(this.$p.permissionChecker('adminAssistantSecurity') && this.loading == false){
				this.loading = true
                this.preloader(true)
                
                this.postData.data = JSON.stringify(this.postForm)

                var result = this.$m.postMethod('management/admin/assistant/DBsecurity',this.postData)
			...
			if(this.$p.permissionChecker('userMemberStatus') && this.loading == false){
				...
				this.$confirm(this.$t('msg.msg_confirmation'), this.$t('msg.prompt'), {
					...
					var result = this.$m.postMethod('management/admin/assistant/DBstatus',this.postData)
			...
			if(this.$p.permissionChecker('adminAssistantPermission') && this.loading == false){
				this.loading = true;
				this.submitForm.master_id = id
				this.postForm.username = login
				this.postPermission.master_id = id
				this.postData.data = JSON.stringify(this.submitForm);
				var result = this.$m.postMethod("management/admin/assistant/permission", this.postData)
				result.then((value) => {
					var data = value.data
			...
			if(this.$p.permissionChecker('adminAssistantPermission') && this.loading == false){
				...
				this.$confirm(this.$t('msg.msg_confirmation'), this.$t('msg.prompt'), {
					...
					var result = this.$m.postMethod("management/admin/assistant/DBpermission", this.postData)
