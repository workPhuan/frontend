<template>
	<div class="page-container">
		<div class="page-header">
			<label>
				<i class="fa-solid fa-arrow-right-to-bracket pe-2"></i> {{$t('menu.management_admin_trace')}}
			</label>
		</div>
			...
			<el-card shadow="never">
				<div class="page-filter">
					...
					<div class="p-3 d-flex flex-wrap">
                        ...
                        <el-input class="custom-input fixed-width-200 m-2" v-model="searchData.action_ip" :placeholder="$t('mix.table_please_enter')+$t('mix.table_action_ip')" @keyup.enter.native="initial()" clearable>
                            <template #prepend>
                                <label>{{$t('mix.table_action_ip')}}</label>
                            </template>
                        </el-input>
						...
						<el-table-column :prop="title.prop" :label="title.label" :min-width="title.width" :align="title.align" :type="title.type">
                            ...
                            <template v-if="title.prop === 'response'" #default="scope">
                                <el-button-group>
                                    ...
                                    <el-button type="primary" @click="copy(scope.row.response),shareCopy()">Copy</el-button>
                                </el-button-group>
                                <div v-if="scope.row.expanded">
                                    {{ scope.row.response }}
                                </div>
                                <!-- <div class="demo-collapse">
                                    <el-collapse @change="toggleExpanded(scope.row)">
                                    <el-collapse-item :title="scope.row.expanded ? 'Expand' : 'Collapse'">
                                        <div>
                                        {{ scope.row.response }}
                                        </div>
                                    </el-collapse-item>
...
export default {
	...
	methods:{
		initial(){
			...
			var result = this.$m.postMethod('management/admin/trace/ajaxTable',this.postData)
			result.then((value) => {
				var data = value.data
